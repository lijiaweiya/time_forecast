...
from cluster_test import main_clustering_analysis, reshape_time_series_data
...
class Dataset_ETT_minute(Dataset):
    ...
    def __read_data__(self):
        ...
        if os.path.exists(model_path):
            self.cluster = joblib.load(model_path)
        else:
            self.cluster = KMeans(n_clusters=self.args.cluster, random_state=2001, n_init=10)
        ...
        #     x_all.append(dataset[i][0])
        # x_all = np.array(x_all)
        # print(x_all.shape)
        # X = x_all
        # _, kmeans_labels = main_clustering_analysis(X, algorithm='kmeans', use_dtw=False)
        # self.external_labels = kmeans_labels
        # # 加载外部标签
        # if self.classification and self.external_labels is not None:
        #     # 确保外部标签长度与数据长度匹配
        #     if len(self.external_labels) != len(df_raw):
        #         print(f"警告: 外部标签长度({len(self.external_labels)})与数据长度({len(df_raw)})不匹配")
        #         # 截取或填充标签以匹配数据长度
        #         if len(self.external_labels) < len(df_raw):
        #             # 填充标签
        #             padding = np.zeros(len(df_raw) - len(self.external_labels), dtype=self.external_labels.dtype)
        #             self.external_labels = np.concatenate([self.external_labels, padding])
        #         else:
        #             # 截取标签
        #             self.external_labels = self.external_labels[:len(df_raw)]
        #
        #     # 根据当前数据集分割截取标签
        #     self.label_y = self.external_labels[border1:border2]
        #
        #     # 验证标签长度
        #     print(f"外部标签长度: {len(self.external_labels)}")
        #     print(f"当前数据集分割标签长度: {len(self.label_y)}")
        #     print(f"标签唯一值: {np.unique(self.label_y)}")
        #     print(f"标签分布: {np.bincount(self.label_y)}")
        # x_all=[]
    ...
    def geti(self, index):
        ...
        return seq_x, seq_x_mark, seq_y_mark
